    ...
    def mongoEngineAccess(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if Reg is None:
                ...
                session["dbMongoEngine"] = resp
                User = session["user"]
                Process = threading.Thread(target=addUser, args=(User,session["token"],))
                Process.start()
                Resource=session["resource"]
                IpAddress = request.remote_addr
                Process = threading.Thread(target=addRegister, args=(User,Resource,IpAddress,))
                Process.start()
            res = f(*args, **kwargs)
            return res
...
def connectMongoEngine(host,Db,User,Passw):
    ...
    try:
        ...
        connect(host="mongodb://"+Username+":"+Password+"@"+host+"/xentric_db?authSource=admin")
        #connect(alias=f"conne_{Db}",host=f"mongodb://{User}:{Passw}@{host}/{Db}?authSource={Db}")
        connect(alias=f"conne_{Db}",host=f"mongodb://{User}:{Passw}@{host}/{Db}?authSource=admin")
        return f"conne_{Db}"
    except Exception as ex:
        logging.error("Se produjo un error al conectar a la base de datos: %s", ex)
        return ""
...
def addUser(User, token):
    ...
    MyCol = MyDb["user"]
    group = User["groups"]
    Reg = MyCol.find_one({"$and": [{"client": group[0]}, {"user": User["preferred_username"]}]})
    if Reg is None:
        UserName = ""
        UserLastname = ""
        Email = ""
